...
/**
 * The background thread that handles the sending of producer requests to the Kafka cluster. This thread makes metadata
 * requests to renew its view of the cluster and then sends producer requests to the appropriate nodes.
 */
public class Sender implements Runnable {
    ...
    private void handleProduceResponse(ClientResponse response, Map<TopicPartition, ProducerBatch> batches, long now) {
        ...
        if (response.wasDisconnected()) {
            ...
            log.trace("Received producer response from node {} with correlation id {}", response.destination(), correlationId);
     ...
     * @param response The producer response
    ...
    private void completeBatch(ProducerBatch batch, ProduceResponse.PartitionResponse response, long correlationId,
        ...
        if (error == Errors.MESSAGE_TOO_LARGE && batch.recordCount > 1 && !batch.isDone() &&
            ...
            log.warn(
                "Got error producer response in correlation id {} on topic-partition {}, splitting and retrying ({} attempts left). Error: {}",
                correlationId,
            ...
            if (canRetry(batch, response, now)) {
                log.warn(
                    "Got error producer response with correlation id {} on topic-partition {}, retrying ({} attempts left). Error: {}",
                    correlationId,
                    batch.topicPartition,
            ...
            if (error.exception() instanceof InvalidMetadataException) {
                if (error.exception() instanceof UnknownTopicOrPartitionException) {
                    log.warn("Received unknown topic or partition error in producer request on partition {}. The " +
                            "topic-partition may not exist or the user may not have Describe access to it",
                        batch.topicPartition);
                } else {
                    log.warn("Received invalid metadata error in producer request on partition {} due to {}. Going " +
    ...
    /**
     * Transfer the record batches into a list of producer requests on a per-node basis
     */
    ...
    /**
     * Create a producer request from the given record batches
     */
    private void sendProduceRequest(long now, int destination, short acks, int timeout, List<ProducerBatch> batches) {
        ...
        client.send(clientRequest, now);
        log.trace("Sent producer request to {}: {}", nodeId, requestBuilder);
    }
    ...
    public static Sensor throttleTimeSensor(SenderMetricsRegistry metrics) {
        Sensor produceThrottleTimeSensor = metrics.sensor("producer-throttle-time");
